---
title: "Structuring of TRACE raw files into a SQLiteDB"
...
The analysis of bond trading is an interesting and emerging field in the finance literature. 
Data for bond trades is usually available for the US market and provided by FINRA, and published via two sources, [PLAYER 1]() and [TRACE/FINRA]().
The Trade Reporting and Compliance Engine (TRACE) data, published by FINRA, is more affordable in terms of pricing; 
however, it comes as a set of nested ZIP files and TXT files in two different formats, which makes analysis and processing hard.

Fortunately, we came up with a solution to convert the set of many files into one normalized database schema, that enables efficient and powerful analysis.
Within this blog post we want to show you how you can leverage our [R code](https://github.com/DerKevinRiehl/bondTradingDataSqliteDbConverterR) to 
process and analyze bond trading data from TRACE and convert it into a SQLiteDB, that can be used by a variety of applications and programming languages
for downstream analysis.

In the following, we will discuss (1) the structure of bond trading data "TRACE" provided by FINRA, 
(2) the proposed, normalized database schema for the generated SQLiteDB database,
(3) a short hands-on tutorial on how to download data and use our [R code](https://github.com/DerKevinRiehl/bondTradingDataSqliteDbConverterR) to generate the SQLiteDB with our code, and 
(4) an overview of the number of bond trades we found for our downloaded copy of the FINRA TRACE data.
...
First of all, you need to get a copy of the TRACE (FINRA) data. EXPLAIN WHERE TO GET IT; WHICH WEBSITE; HOW TO PAY/ORDER AND DOWNLOAD.
**Please note** that storing the downloaded copy on your local disk might require some more disk space (HOW MUCH?).
...
**Please note**, that the conversion can take some time (HOW MUCH TIME) and might require disk space (HOW MUCH SPACE).
...
zip_file_B_20 <- ("EHDwC 2020.zip")
zip_file_B_21 <- ("EHDwC 2021.zip")
zip_file_B_22 <- ("EHDwC 2022.zip")
convert_downloaded_enhanced_data_to_sqlite_db(conn, zip_file_B_20)
convert_downloaded_enhanced_data_to_sqlite_db(conn, zip_file_B_21)
convert_downloaded_enhanced_data_to_sqlite_db(conn, zip_file_B_22)
...
## (4) Empirical overview of TRACE bond trading data
@LUKAS PROVIDE SOME TEXT AND SUMMARY STATISTICS, LIKE HOW MUCH ACADEMIC AND ENHANCED data (number of records) you found for each year, how much trade volume in MILLION USD,
MAKE SOME FIGURES OUT OF IT...
...
and Prof. Dr. Dirk Schiereck from the Chair of Corporate Finance who made the data available for this project.
