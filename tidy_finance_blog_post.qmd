...
Our R code enables you organizing the TRACE academic data and TRACE enhanced data by eliminating the complexities of multiple, nested files, simplifying the data conversion process. Drawing from the existing SAS-based solution of Dick-Nielsen (2014) [^1] and (2019) [^2], you can now easily convert TRACE data into a single, organized SQLiteDB database, allowing for seamless and efficient downstream analysis.
...
In the following, we will discuss first, the structure of the bond trading data "TRACE" provided by FINRA, second the proposed normalized database schema for the generated SQLiteDB database, and finally third, give you a brief hands-on guide to downloading data and using our [R code] (https://github.com/DerKevinRiehl/bondTradingDataSqliteDbConverterR) to generate the SQLiteDB. 
...
After successfully ordering and downloading the data, you should possess all necessary raw files to start creating a SQLiteDB. In the following, we assume that you possess a folder with all downloaded TRACE data (enhanced and academic) called "TRACE_Downloads", with a similar structure as follows:
```
TRACE_Downloads/
├── academic_txts/
...
The below R code allows the user to convert the raw data provided by FINRA (Academic Data as well as Enhanced Historical Data) into a SQLiteDB for further analyses. For the curious reader, you can find implementation details and code documentation in our [GitHub Repository](https://github.com/DerKevinRiehl/bondTradingDataSqliteDbConverterR). 
